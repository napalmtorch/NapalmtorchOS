...
vfs_directory_t vfs_dir_info(const char* path)
{
    ...
    out_dir.name = calloc(strlen(name) + 1);
    strcpy(out_dir.name, name);
    
    // copy name and path
    if (dir_name != NULL)
    {
        out_dir.path = calloc(strlen(dir_name) + 1);
        strcpy(out_dir.path, dir_name);
    }
    else { out_dir.path = calloc(1); out_dir.path[0] = 0; }
...
vfs_file_t vfs_file_info(const char* path)
{
    ...
    vfs_file_t out_file;
    out_file.name = calloc(strlen(name) + 1);
    strcpy(out_file.name, name);
    
    // copy name and path
    if (dir_name != NULL)
    {
        out_file.path = calloc(strlen(dir_name) + 1);
        strcpy(out_file.path, dir_name);
    }
    else { out_file.path = calloc(1); out_file.path[0] = 0; }
...
{
    fs_directory_t dir = fs_get_dir_byname(path);
    if (dir.type != FSTYPE_DIR) { debug_printf("Error while counting directories in '%s'\n", path); return 0; }
    int index = fs_get_dir_index(dir);

    uint32_t dirs_count = 0;
    uint8_t* secdata = calloc(ATA_SECTOR_SIZE);
    for (uint32_t sec = 0; sec < fs_get_info().file_table_max_used; sec++)
    {
        atapio_read(fs_get_info().file_table_start + sec, 1, secdata);
...
{
    if (!vfs_dir_exists(path)) { debug_printf("Unable to count files in '%s'\n", path); return 0; }

    fs_directory_t dir = fs_get_dir_byname(path);
    if (dir.type != FSTYPE_DIR) { debug_printf("Error while counting files in '%s'\n", path); return 0; }
    int index = fs_get_dir_index(dir);
...
{
    if (!vfs_dir_exists(path)) { debug_printf("Unable to locate directory '%s'\n", path); return NULL; }

    fs_directory_t dir = fs_get_dir_byname(path);
    if (dir.type != FSTYPE_DIR) { debug_printf("Error while locating directory '%s'\n", path); return NULL; }
    int index = fs_get_dir_index(dir);
    ...
    for (uint32_t sec = 0; sec < fs_get_info().file_table_max_used; sec++)
    {
        ...
        for (uint32_t i = 0; i < ATA_SECTOR_SIZE; i += sizeof(fs_directory_t))
        {
            ...
            if (tempdir->type == FSTYPE_DIR && tempdir->parent_index == index)
            {
                char* dirname = calloc(strlen(tempdir->name) + 1);
...
char** vfs_get_files(const char* path, int* count)
{
    ...
    for (uint32_t sec = 0; sec < fs_get_info().file_table_max_used; sec++)
    {
        ...
        for (uint32_t i = 0; i < ATA_SECTOR_SIZE; i += sizeof(fs_directory_t))
        {
            ...
            {
                char* fname = calloc(strlen(tempfile->name) + 1);
...
uint8_t* vfs_read_bytes(const char* path, uint32_t* count)
{
    fs_file_t file = fs_file_read(path);
    if (file.type != FSTYPE_FILE) { return NULL; }
    *count = file.size;
...
bool_t vfs_delete_dir(const char* path, bool_t recursive)
{
    ...
    {
        debug_printf("Recursive delete not yet implemented\n");
        return FALSE;
    }
}
...
{
    if (recursive) { debug_printf("Recursive copy not yet implemented\n"); return FALSE; }
