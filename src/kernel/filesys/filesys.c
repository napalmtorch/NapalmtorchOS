...
void fs_format(uint32_t size, bool_t wipe)
{
    ...
    memsetl(fs_blk_mass.padding, 0, sizeof(fs_blk_mass.padding));
    fs_blktable_write(0, fs_blk_mass);
    fs_blk_mass = fs_blktable_read(0);
    debug_printf("Created mass block: START: %d, STATE = 0x%2x, COUNT = %d\n", fs_blk_mass.start, fs_blk_mass.state, fs_blk_mass.count);

    // create files block entry and update info
    fs_info_read();
    fs_info.file_table_count_max = 16384;
    ...
    // finished
    debug_printf("Finished formatting disk\n");
...
{
    debug_printf("Started wiping disk...\n");
    uint32_t sectors = size / ATA_SECTOR_SIZE;
    uint8_t* data = calloc(ATA_SECTOR_SIZE);
    memsetl(data, 0, ATA_SECTOR_SIZE);

    for (uint64_t i = 0; i < sectors; i++) { atapio_write(i, 1, data); }

    debug_printf("Finished wiping disk\n");
    free(data);
}
...
void fs_info_create(uint32_t size)
{
    ...
    fs_info.blk_table_count_max = 65536;
    ...
    // finished
    debug_printf("Created new info block\n");
}

// read info block from disk
void fs_info_read()
{
    uint8_t* sec = calloc(ATA_SECTOR_SIZE);
...
{
    debug_printf("PRINTING BLOCK TABLE: \n");
    ...
    for (uint32_t sec = 0; sec < fs_info.blk_table_sector_count; sec++)
    {
        ...
        for (uint32_t i = 0; i < ATA_SECTOR_SIZE; i += sizeof(fs_blkentry_t))
        {
            ...
            debug_printf("INDEX: 0x%8x START: 0x%8x COUNT: 0x%8x STATE: 0x%2x\n", index, entry->start, entry->count, entry->state);
            index++;
        }
    }
...
{
    fs_info_read();
    if (index < 0 || index >= fs_info.blk_table_count_max) { debug_printf("Invalid index while reading from block table\n"); return NULL_BLKENTRY; }
    ...
    for (uint32_t sec = 0; sec < fs_info.blk_table_sector_count; sec++)
    {
        ...
        for (uint32_t i = 0; i < ATA_SECTOR_SIZE; i += sizeof(fs_blkentry_t))
        {
            ...
            if (entry->count == sectors && entry->state == FSSTATE_FREE)
            {
                ...
                debug_printf("Allocated block: START: 0x%8x, STATE = 0x%2x, COUNT = 0x%8x\n", output.start, output.state, output.count);
                free(data);
                return output;
            }
...
bool_t fs_blktable_free(fs_blkentry_t entry)
{
    ...
    for (uint32_t sec = 0; sec < fs_info.blk_table_sector_count; sec++)
    {
        ...
        for (uint32_t i = 0; i < ATA_SECTOR_SIZE; i += sizeof(fs_blkentry_t))
        {
            ...
            if (temp->start == entry.start && temp->count == entry.count && temp->state == entry.state)
            {
                ...
                debug_printf("Freed block: START: 0x%8x, STATE = 0x%2x, COUNT = 0x%8x\n", temp->start, temp->state, temp->count);
    ...
    debug_printf("Unable to free block START: %d, STATE = 0x%2x, COUNT = %d\n", entry.start, entry.state, entry.count);
    return FALSE;
}
...
void fs_blktable_merge_free()
{
    ...
    for (uint32_t sec = 0; sec < fs_info.blk_table_sector_count; sec++)
    {
        ...
        for (uint32_t i = 0; i < ATA_SECTOR_SIZE; i += sizeof(fs_blkentry_t))
        {
            ...
            if (temp->state == FSSTATE_FREE)
            {
                ...
                {
                    debug_printf("TEMP: 0x%8x, NEAREST: 0x%8x\n", temp->start, nearest.start);
                    if (temp->start > nearest.start) { temp->start = nearest.start; }
                    temp->count += nearest.count;
                    atapio_write(fs_info.blk_table_start + sec, 1, data);
                    if (!fs_blktable_delete_entry(nearest)) { debug_printf("Error deleting entry while merging\n"); return; }
                    atapio_read(fs_info.blk_table_start + sec, 1, data);
                }
            }
            index++;
    ...
    for (uint32_t sec = 0; sec < fs_info.blk_table_sector_count; sec++)
    {
        ...
        for (uint32_t i = 0; i < ATA_SECTOR_SIZE; i += sizeof(fs_blkentry_t))
        {
            ...
            {
                debug_printf("MASS: START = 0x%8x, COUNT = 0x%8x, STATE = 0x%2x\n", mass.start, mass.count, mass.state);
                debug_printf("TEMP: START = 0x%8x, COUNT = 0x%8x, STATE = 0x%2x\n", temp->start, temp->count, temp->state);
                mass.start = temp->start;
                mass.count += temp->count;
                mass.state = FSSTATE_FREE;
                fs_blktable_write(0, mass);
                atapio_read(fs_info.blk_table_start + sec, 1, data);
                memsetl(temp, 0, sizeof(fs_blkentry_t));
                atapio_write(fs_info.blk_table_start + sec, 1, data);
                fs_info_write();
                break;
            }
...
fs_blkentry_t fs_blktable_create_entry(uint32_t start, uint32_t count, uint8_t state)
{
    ...
    if (i < 0 || i >= fs_info.blk_table_count_max) { debug_printf("Maximum amount of block entries reached\n"); return NULL_BLKENTRY; }
    fs_blkentry_t entry;
    entry.start = start;
    entry.count = count;
    entry.state = state;
    memsetl(entry.padding, 0, sizeof(entry.padding));
    fs_blktable_write(i, entry);
    fs_info.blk_table_count++;
    fs_info_write();
    //debug_printf("Created block: START: %d, STATE = 0x%2x, COUNT = %d\n", entry.start, entry.state, entry.count);
    return entry;
}
...
bool_t fs_blktable_delete_entry(fs_blkentry_t entry)
{
    ...
    for (uint32_t sec = 0; sec < fs_info.blk_table_sector_count; sec++)
    {
        ...
        for (uint32_t i = 0; i < ATA_SECTOR_SIZE; i += sizeof(fs_blkentry_t))
        {
            ...
            if (temp->start == entry.start && temp->count == entry.count && temp->state == entry.state)
            {
                debug_printf("Delete block: START: 0x%8x, STATE = 0x%2x, COUNT = 0x%8x\n", entry.start, entry.state, entry.count);
                memsetl(temp, 0, sizeof(fs_blkentry_t));
                atapio_write(fs_info.blk_table_start + sec, 1, data);
                fs_info.blk_table_count--;
                fs_info_write();
    ...
    debug_printf("Unable to delete block\n");
    return FALSE;
}
...
bool_t fs_blktable_fill(fs_blkentry_t entry, uint8_t value)
{
    ...
    for (uint32_t sec = 0; sec < fs_info.blk_table_sector_count; sec++)
    {
        ...
        for (uint32_t i = 0; i < ATA_SECTOR_SIZE; i += sizeof(fs_blkentry_t))
        {
            ...
            if (temp->start == entry.start && temp->count == entry.count && temp->state == entry.state)
            {
                atapio_fill(temp->start, temp->count, value);
                free(data);
                return TRUE;
            }
        }
    ...
    debug_printf("Unable to fill block entry\n");
    free(data);
    return FALSE;
}
...
fs_blkentry_t fs_blktable_at_index(int index)
{
    ...
    debug_printf("Unable to get block entry at index %d\n", index);
    free(data);
    return NULL_BLKENTRY;
}
...
bool_t fs_root_create(const char* label)
{
    ...
    debug_printf("Created root: NAME = %s, PARENT = 0x%8x, TYPE = 0x%2x, STATUS = 0x%2x\n", rfd.name, rfd.parent_index, rfd.type, rfd.status);
    return TRUE;
}

void fs_filetable_print()
{
    debug_printf("------ FILE TABLE ----------------------------\n");
    ...
    for (uint32_t sec = 0; sec < fs_info.file_table_sector_count; sec++)
    {
        ...
        for (uint32_t i = 0; i < ATA_SECTOR_SIZE; i += sizeof(fs_directory_t))
        {
            ...
            {
                debug_printf("INDEX: 0x%8x PARENT: 0x%8x TYPE: 0x%2x STATUS: 0x%2x NAME: %s\n", index, entry->parent_index, entry->type, entry->status, entry->name);
            }
            index++;
        }
    }
...
{
    fs_info_read();
    if (index < 0 || index >= fs_info.file_table_count_max) { debug_printf("Invalid index while reading directory entry\n"); return NULL_DIR; }
...
{
    fs_info_read();
    if (index < 0 || index >= fs_info.file_table_count_max) { debug_printf("Invalid index while reading file entry\n"); return NULL_FILE; }
    uint32_t sector = fs_filetable_sector_from_index(index);
    uint32_t offset = fs_filetable_offset_from_index(sector, index);
    uint8_t* data = calloc(ATA_SECTOR_SIZE);
    atapio_read(sector, 1, data);
...
{
    fs_info_read();
    if (index < 0 || index >= fs_info.file_table_count_max) { debug_printf("Invalid index while writing directory entry\n"); return; }
    uint32_t sector = fs_filetable_sector_from_index(index);
    uint32_t offset = fs_filetable_offset_from_index(sector, index);
    uint8_t* data = calloc(ATA_SECTOR_SIZE);
    atapio_read(sector, 1, data);
...
{
    fs_info_read();
    if (index < 0 || index >= fs_info.file_table_count_max) { debug_printf("Invalid index while writing file entry: %d\n", index); return; }
    uint32_t sector = fs_filetable_sector_from_index(index);
    uint32_t offset = fs_filetable_offset_from_index(sector, index);
    uint8_t* data = calloc(ATA_SECTOR_SIZE);
    atapio_read(sector, 1, data);
...
fs_directory_t fs_filetable_create_dir(fs_directory_t dir)
{
    ...
    if (i < 0 || i >= fs_info.file_table_count_max) { debug_printf("Invalid index while creating directory entry\n"); return NULL_DIR; }
    fs_filetable_write_dir(i, dir);
    fs_info.file_table_count++;
    fs_info_write();
    debug_printf("Created directory: INDEX = 0x%8x, NAME = %s, PARENT = 0x%8x, TYPE = 0x%2x, STATUS = 0x%2x\n", i, dir.name, dir.parent_index, dir.type, dir.status);
    return dir;
}
...
{
    int i = fs_filetable_freeindex();
    if (i < 0 || i >= fs_info.file_table_count_max) { debug_printf("Invalid index while creating file entry\n"); return NULL_FILE; }
    fs_filetable_write_file(i, file);
    fs_info.file_table_count++;
    fs_info_write();
    debug_printf("Created file: NAME = %s, PARENT = 0x%8x, TYPE = 0x%2x, STATUS = 0x%2x, BLK = 0x%8x, SIZE = %d\n", file.name, file.parent_index, file.type, file.status, file.blk_index, file.size);
    return file;
}
...
bool_t fs_filetable_delete_dir(fs_directory_t dir)
{
    ...
    uint8_t* data = calloc(ATA_SECTOR_SIZE);
    int index = 0;
    for (uint32_t sec = 0; sec < fs_info.file_table_max_used; sec++)
    {
        atapio_read(fs_info.file_table_start + sec, 1, data);
        ...
        for (uint32_t i = 0; i < ATA_SECTOR_SIZE; i += sizeof(fs_directory_t))
        {
            ...
            {
                debug_printf("Deleted directory: NAME = %s, PARENT = 0x%8x, TYPE = 0x%2x, STATUS = 0x%2x\n", dir.name, dir.parent_index, dir.type, dir.status);
                memsetl(entry, 0, sizeof(fs_directory_t));
    ...
    debug_printf("Unable to delete directory\n");
    free(data); 
    return FALSE;
}
...
bool_t fs_filetable_delete_file(fs_file_t file)
{
    ...
    for (uint32_t sec = 0; sec < fs_info.file_table_max_used; sec++)
    {
        ...
        for (uint32_t i = 0; i < ATA_SECTOR_SIZE; i += sizeof(fs_file_t))
        {
            ...
            {
                debug_printf("Deleted file: NAME = %s, PARENT = 0x%8x, TYPE = 0x%2x, STATUS = 0x%2x, SIZE = %d\n", file.name, file.parent_index, file.type, file.status, file.size);
                memsetl(entry, 0, sizeof(fs_file_t));
                atapio_write(fs_info.file_table_start + sec, 1, data);
                free(data); 
                return TRUE;
            }
...
fs_directory_t fs_parent_from_path(const char* path)
{
    ...
    debug_printf("Unable to locate parent of %s\n", path);
...
fs_file_t fs_get_file_byname(const char* path)
{
    ...
    if (parent.type != FSTYPE_DIR) { debug_printf("Parent was null while getting file by name\n"); return NULL_FILE; }
    ...
    if (strlen(filename) == 0 || filename == NULL) { freearray(args, args_count); debug_printf("Unable to get name while getting file by name\n"); return NULL_FILE; }
...
fs_directory_t fs_get_dir_byname(const char* path)
{
    ...
    fs_directory_t parent = fs_parent_from_path(path);
    if (parent.type != FSTYPE_DIR) { debug_printf("Parent was null while getting directory by name\n"); return NULL_DIR; }

    int args_count = 0;
    char** args = strsplit(path, '/', &args_count);

    if (args_count == 0) { freearray(args, args_count); debug_printf("Args was null while getting directory by name\n"); return NULL_DIR; }
    ...
    if (strlen(dirname) == 0 || dirname == NULL) { debug_printf("Unable to get name while getting directory by name\n"); return NULL_DIR; }
...
char* fs_get_name_from_path(const char* path)
{
    ...
    if (!strcmp(path, "/"))
    {
        char* rootname = calloc(strlen(fs_rootdir.name) + 1);
        memsetl(rootname, 0, strlen(fs_rootdir.name));
    ...
    if (args_count == 0) { freearray(args, args_count); debug_printf("Args was null while getting name from path\n"); return NULL; }

    if (args_count == 2 && path[0] == '/')
    {
        char* rootname = calloc(strlen(args[1]) + 1);
        memsetl(rootname, 0, strlen(args[1]) + 1);
    ...
    char* output = calloc(strlen(filename) + 1);
    memsetl(output, 0, strlen(filename));
...
char* fs_get_parent_path_from_path(const char* path)
{
    ...
    char* output = calloc(strlen(path + 16));
    memsetl(output, 0, strlen(path + 16));
...
{
    if (size == 0) { debug_printf("Cannot create blank file\n"); return NULL_FILE; }

    fs_directory_t parent = fs_parent_from_path(path);
    if (parent.type != FSTYPE_DIR) { debug_printf("Unable to locate parent while creating file\n"); return NULL_FILE; }
...
{
    if (path == NULL) { debug_printf("Path was null while trying to read file %s\n", path); return NULL_FILE; }
    if (strlen(path) == 0) { debug_printf("Path was empty while trying to read file %s\n", path); return NULL_FILE; }

    fs_file_t file = fs_get_file_byname(path);
    if (file.type != FSTYPE_FILE) { debug_printf("Unable to locate file %s", path); return NULL_FILE; }

    fs_blkentry_t blk = fs_blktable_read(file.blk_index);

    uint8_t* data = calloc(blk.count * ATA_SECTOR_SIZE);
    for (uint32_t i = 0; i < blk.count; i++) { atapio_read(blk.start + i, 1, data + (i * ATA_SECTOR_SIZE)); }
    file.data = data;
    return file;
}
...
{
    if (path == NULL) { debug_printf("Path was null while trying to write file %s\n", path); return FALSE; }
    if (strlen(path) == 0) { debug_printf("Path was empty while trying to write file %s\n", path); return FALSE; }

    fs_file_t tryload = fs_get_file_byname(path);
    if (tryload.type != FSTYPE_FILE) 
    { 
        debug_printf("File %s does not exist and will be created\n", path);
        fs_file_t new_file = fs_file_create(path, len);
        if (new_file.type != FSTYPE_FILE) { debug_printf("Unable to create new file %s\n", path); return FALSE; }
        fs_blkentry_t blk = fs_blktable_read(new_file.blk_index);

        uint8_t* secdata = calloc(ATA_SECTOR_SIZE);
        for (uint32_t i = 0; i < blk.count; i++)
        {
            memsetl(secdata, 0, ATA_SECTOR_SIZE);
        ...
        debug_printf("Written file %s to disk, size = %d\n", path, new_file.size);
        free(secdata);
        return TRUE;
    }
    else 
